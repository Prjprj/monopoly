...
void casespecial(Joueur *slayer, card *tab, int *play, listeJoueur *stuch) {
	...
	case recul:
		deplacementde(NBCASESPLATEAU - (tab->parametre % NBCASESPLATEAU),
				slayer, tab->dep);
		*play = 1;
		break;
	...
	case avancecase:
		if (slayer->courant->numero <= tab->parametre) {
			deplacementde(tab->parametre - slayer->courant->numero, slayer,
					tab->dep);
		} else {
			deplacementde(
			NBCASESPLATEAU + tab->parametre - slayer->courant->numero, slayer,
					tab->dep);
		}
		*play = 1;
		break;
