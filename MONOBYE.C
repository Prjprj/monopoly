...
Joueur* byebye(Joueur *slayer){
	...
	for (i=0; i<NBPROPRIETE; i++){
		if (tableaupropriete[i].proprietaire != NULL && tableaupropriete[i].proprietaire->numero == slayer->numero){
			tableaupropriete[i].proprietaire = NULL;
			tableaupropriete[i].nbmaison = 0;
		}
	}
	for (i=0; i<NBGARES; i++){
		if (tableaugare[i].proprietaire != NULL && tableaugare[i].proprietaire->numero == slayer->numero){
			tableaugare[i].proprietaire = NULL;
		}
	}
	for (i=0; i<NBCOMPAGNIE; i++){
		if (tableaucompagnie[i].proprietaire != NULL && tableaucompagnie[i].proprietaire->numero == slayer->numero){
