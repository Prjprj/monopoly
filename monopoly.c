...
void monopoly(void){
	...
	initplateau("monopoly.dat",&casedepart, tableaupropriete, tableaugare, tableaucompagnie);
	...
	if(rep== 1){
		...
		for(i=0;i<monoplayers.nombrejoueur;i++){
			...
			dialogCharBox(moi->nom, rien1,30,9,20, BROUGE|FBLEU, BTURQ|FVIOLET);
	...
	do{
		...
		if (moi->prison != 0){
			...
			if (faux == menutour(moi, &monoplayers, tableaupropriete, tableaucouleur, tableaugare, tableaucompagnie, &replay)){
				replay = 1;
				c = faux;
			}
		}
		...
		while(replay == 0 || replay == -1){
			...
			if (des < 0 ){
				for(i=1;i<25;i++)
				{
					colorlocate("!!!!DOUBLE!!!!",65,i,(WORD)(colorLettreHasard()|16*colorLettreHasard()));
					colorlocate("!!!!DOUBLE!!!!",51,i,(WORD)(colorLettreHasard()|16*colorLettreHasard()));
					sleep(0.05);
				} 
				getche();
				replay = 0;
            ...
            if (moi->prison == 0){
				...
				do{
					pasfini = 0;
					deplacementsurcase(moi, &monoplayers, tableaupropriete, tableaucouleur, tableaugare, tableaucompagnie, tabchance, tabcaisse, &pasfini, des);
