...
void cls(void)
{
	...
	FillConsoleOutputCharacter(hStdOut,(TCHAR) ' ',tailleEcran, origine, &nombreCaracteresEcrits);

	// Remplir l'ecran avec la couleur courante

	FillConsoleOutputAttribute(hStdOut, csbi.wAttributes, tailleEcran, origine, &nombreCaracteresEcrits);
...
int locate(char chaine[80], int x, int y)
{
	...
	curseur.X = x;
	curseur.Y = y;
	WriteConsoleOutputCharacter(hStdOut, chaine, strlen(chaine), curseur, &nombreCaracteres);
    return 1;
}
...
int menubox(menu *current){
	...
	box((WORD)current->boxcolor,(int)current->x,(int)current->y,(int)longchaine+3,(int)2*current->taille);
	for(i = 0; i<(*current).taille;i++){
		colorlocate(current->chaine[i],current->x+2,current->y+1+2*i,(WORD)current->colorelement);
		color((WORD)current->colorelement,current->x+1,current->y+1+2*i,longchaine+2);
	}
	while(c==-1){
		colorlocate(current->chaine[keycours],current->x+2,current->y+1+2*keycours,(WORD)current->colorchoix);
		color((WORD)current->colorchoix,current->x+1,current->y+1+2*keycours,longchaine+2);
		key = getkeyblock();
		if (key == 302){
			colorlocate(current->chaine[keycours],current->x+2,current->y+1+2*keycours,(WORD)current->colorelement);
			color((WORD)current->colorelement,current->x+1,current->y+1+2*keycours,longchaine+2);
...
int dialogIntBox(char c[80], int x, int y, int taille, WORD couleurboite, WORD couleurchoix)
{
	int rep;
	COORD point;
	point.X = x+2;
...
void dialogCharBox(char *reponse, char c[80], int x, int y, int taille, WORD couleurboite, WORD couleurchoix)
{
	COORD point;
