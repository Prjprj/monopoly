...
int lancede(void){     //les des 
	...
	c = 0;
	cadre("?",7,7,11,11,(WORD)(colorLettreHasard()|16*colorLettreHasard()));
	cadre("?",32,7,11,11,(WORD)(colorLettreHasard()|16*colorLettreHasard()));
	while (c != 13){
		de.de1 = (int)(rnd()*6 + 1);
...
int monomenu(menum *current){
	...
	box((WORD)current->boxcolor,(int)current->x,(int)current->y,(int)longchaine+3,(int)current->taille+1);
	for(i = 0; i<current->taille;i++){
		colorlocate(current->chaine[i],current->x+2,current->y+1+i,(WORD)current->colorelement);
		color((WORD)current->colorelement,current->x+1,current->y+1+i,longchaine+2);
	}
	while (c==-1){
		colorlocate(current->chaine[keycours],current->x+2,current->y+1+keycours,(WORD)current->colorchoix);
		color((WORD)current->colorchoix,current->x+1,current->y+1+keycours,longchaine+2);
		key = getkeyblock();
		if (key == 302){
			colorlocate(current->chaine[keycours],current->x+2,current->y+1+keycours,(WORD)current->colorelement);
			color((WORD)current->colorelement,current->x+1,current->y+1+keycours,longchaine+2);
