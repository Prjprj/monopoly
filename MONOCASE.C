...
void deplacementsurcase(Joueur *moi, listeJoueur *stach, propriete *tabprop, int *tabcolor, station *tabgare, compagnies *tabcomp, card *tab1, card *tab2, int *pasfini, int des){
	int cartechance;
	switch (moi->courant->type){
		...
		case terrain:
			...
			if (casepropriete(moi,&tabprop[moi->courant->place], tabcolor)== faux){
